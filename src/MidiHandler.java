...
public class MidiHandler {
    ...
    public class MidiInputReceiver implements Receiver {
        ...
        public void send(MidiMessage msger, long timeStamp) {

            // if (canPlay) {
            //     canPlay = false;
            byte[] msg = msger.getMessage();

            int len = msger.getLength();
            if (len <= 1) {
                return;
            }

            if(msg[2] >= 127) {
                //System.out.println(msger.getStatus());
                char key = Data.DATA[msg[1] - 36];
